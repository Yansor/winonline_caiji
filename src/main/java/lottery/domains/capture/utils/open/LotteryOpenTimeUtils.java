 ...
 public class LotteryOpenTimeUtils
 {
   ...
   public String getCurrExpect(String lotteryName)
   {
     ...
     for (int i = 0; i < list.size(); i++) {
       if (i < list.size() - 1) {
         LotteryOpenTime ot1 = (LotteryOpenTime)list.get(i);
         LotteryOpenTime ot2 = (LotteryOpenTime)list.get(i + 1);
         if ((curTime.compareTo(ot1.getOpenTime()) >= 0) &&
           (curTime.compareTo(ot2.getOpenTime()) < 0))
         {
           String expect = toDay.replace("-", "") + "-" + ot1.getExpect();
           return expect; }
         if ((curTime.compareTo(ot1.getOpenTime()) >= 0) &&
           (ot1.getOpenTime().compareTo(ot2.getOpenTime()) > 0)) {
           String yesterDay = m.subtract(1, "d").toSimpleDate();

           String expect = yesterDay.replace("-", "") + "-" + ot1.getExpect();
           return expect;
         }
       } else {
         LotteryOpenTime lotteryOpenTime = (LotteryOpenTime)list.get(list.size() - 1);
         if (lotteryOpenTime.getOpenTime().compareTo(lotteryOpenTime
           .getStartTime()) < 0) {
           String yesterDay = m.subtract(1, "d").toSimpleDate();

           String expect = yesterDay.replace("-", "") + "-" + lotteryOpenTime.getExpect();
           return expect;
         }

         String expect = toDay.replace("-", "") + "-" + lotteryOpenTime.getExpect();
         return expect;
       }
     }
